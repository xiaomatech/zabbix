...
parser.add_argument(
    "action",
    help="discoverzones | counter | zonecounter | zonemaintenancecounter | resolvercounter | socketcounter | incounter | outcounter")
...
if os.path.exists(JSONFILE) and time.time() - os.path.getmtime(
        JSONFILE) <= CACHELIFE:
    f = open(JSONFILE)
    j = json.load(f)
    f.close()
    ...
    for stat in root.findall('./bind/statistics/server/zonestat'):
        j['zonemaintenancecounter'][stat.findtext('./name')] = stat.findtext(
            './counter')

    for view in root.findall('./bind/statistics/views/view'):
        if view.findtext('./name') in ('_default', ):
            for stat in view.findall('./resstat'):
                j['resolvercounter'][stat.findtext('./name')] = stat.findtext(
                    './counter')

    for stat in root.findall('./bind/statistics/server/sockstat'):
        j['socketcounter'][stat.findtext('./name')] = stat.findtext(
            './counter')
