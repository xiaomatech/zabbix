...
class Job(object):
    ...
    def collect_namenode(self):
        ...
        mo = re.search('([0-9]+) files and directories, ([0-9]+) blocks',
                       content)
        ...
        result['dfs_capacity'] = self.regulate_size(dfsmap[
            'Configured Capacity'])
    ...
    def collect_jobtracker(self):
        ...
        content = self.request('http://%s:%d/jobtracker.jsp' % (
            self.args.jobtracker_host, self.args.jobtracker_port))
        result = {}

        mo = re.search('Heap Size is ([0-9.]+ [KMGTP]?B)/([0-9.]+ [KMGTP]?B)',
                       content)
        result['heap_used'] = self.regulate_size(mo.group(1))
        result['heap_total'] = self.regulate_size(mo.group(2))
