...
def process(zapi, host):
    ...
    metrics = {'in': (3, 'bps', False), 'out': (3, 'bps', False)}
    process_metrics(
        zapi,
        host=host,
...
def process_metrics(zapi, host, application, search_key, key_pattern, metrics,
                    name_format, key_format):

    host = zapi.host.get(filter={'host': host})
    hostid = host[0]['hostid']

    application = zapi.application.get(hostids=hostid,
    ...
    for item in items:
        ...
        device_info[mo.group('metric')] = (item['itemid'], item['key_'],
                                           item['params'])
    ...
    for metric, metric_info in metrics.iteritems():
        ...
        if metric not in devices['all']:
            ...
            zapi.item.update(itemid=devices['all'][metric][0], params=params)
